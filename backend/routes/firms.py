...
from models.firm import FirmCreate, FirmInvite, FirmAccept, FirmCheckUser
...
async def get_pending_invite(
    ...
    """Returns the first firm that has a pending invitation for the current advocate's phone."""
    phone = advocate.get("phone", "")
    firm = await db.firms.find_one(
        {"invitations": {"$elemMatch": {"phone": phone, "status": "pending"}}},
        {"_id": 0},
    )
    if not firm:
...
async def check_firm_user(
    body: FirmCheckUser,
    advocate=Depends(get_current_advocate),
    db=Depends(get_db),
):
    """Check if a phone number is a registered LawFlow user and whether they are already in a firm."""
    phone = body.phone.strip()
    invitee = await db.advocates.find_one({"phone": phone}, {"_id": 0, "id": 1, "name": 1})
    if not invitee:
        return {"success": True, "data": {"exists": False, "alreadyInFirm": False, "advocateName": ""}}

    in_firm = await db.firms.find_one(
        {"$or": [
            {"ownerId": invitee["id"]},
            {"members.advocateId": invitee["id"]},
        ]},
        {"_id": 0, "id": 1},
    )
    return {
        ...
        "data": {
            "exists": True,
            "alreadyInFirm": bool(in_firm),
            "advocateName": invitee.get("name", ""),
        },
    }
...
async def get_my_firm(
    ...
    # Enrich each member with advocate's name from the advocates collection
    member_ids = [m["advocateId"] for m in firm.get("members", [])]
    adv_docs = await db.advocates.find(
        {"id": {"$in": member_ids}}, {"_id": 0, "id": 1, "name": 1}
    ).to_list(None)
    name_map = {a["id"]: a.get("name") for a in adv_docs}
    for m in firm.get("members", []):
        m["name"] = name_map.get(m["advocateId"])
...
async def invite_member(
    ...
    # Save in-app notification for the invitee
    invitee_adv = await db.advocates.find_one({"phone": phone}, {"_id": 0, "id": 1})
    if invitee_adv:
        notif = {
            "id": str(uuid.uuid4()),
            "advocateId": invitee_adv["id"],
            "title": "Firm Invitation",
            "body": f"{advocate.get('name', 'A senior advocate')} invited you to join {firm['name']}",
            "type": "firm_invite",
            "data": {"screen": "firm"},
            "read": False,
            "createdAt": datetime.utcnow().isoformat(),
...
async def firm_dashboard(
    ...
    # Fetch advocate names for all members
    advocate_docs = await db.advocates.find(
        {"id": {"$in": member_ids}}, {"id": 1, "name": 1, "phone": 1}
    ).to_list(None)
    advocate_map = {a["id"]: a for a in advocate_docs}
    ...
    for mid in member_ids:
        ...
        adv_info = advocate_map.get(mid, {})
        workload[mid] = {
            "name": adv_info.get("name", member_info.get("name", "")),
            "phone": adv_info.get("phone", member_info.get("phone", "")),
    ...
    # Enrich firm members with names from advocates collection
    enriched_members = []
    for m in firm.get("members", []):
        mid = m["advocateId"]
        adv_info = advocate_map.get(mid, {})
        enriched_members.append({
            **m,
            "name": adv_info.get("name", m.get("name", "")),
            "phone": adv_info.get("phone", m.get("phone", "")),
        })

    return {
        "success": True,
        "data": {
            "firm": {**firm, "members": enriched_members},
