...
export default function ClientFormScreen() {
  ...
  const { addClient, updateClient, getClientById, customPartyTypes, addCustomPartyType, clients, isProUser } = useApp();
  ...
  const handleSave = async () => {
    if (!validate()) return;
    // Phase 27 — Paywall check (only when creating, not editing)
    if (!isEdit && !isProUser && clients.length >= 10) {
      setShowPaywall(true);
      return;
    }
    setSaving(true);
    ...
    if (isEdit && clientId) {
      ...
      addClient(payload);
      router.back(); // Go back to clients list
    }
    setSaving(false);
  };
