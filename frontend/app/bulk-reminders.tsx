...
export default function BulkRemindersScreen() {
  ...
  const { hearings, cases, clients, advocateProfile, sendWhatsAppMessage, isProUser } = useApp();
  const advocateName = advocateProfile?.name ?? 'Advocate';
  const dateStr = useMemo(getTomorrowDateStr, []);

  // Phase 27 — Paywall for free users
  const [showPaywall, setShowPaywall] = useState(!isProUser);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {/* Phase 27 — Paywall: shown on screen open for free users */}
      <PaywallSheet
        visible={showPaywall}
        onClose={() => {
        ...
        featureName="Bulk WhatsApp Reminders"
      />
    </SafeAreaView>
  );
}
