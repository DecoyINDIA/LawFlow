...
import { ecourtsLookup, assignCase as assignCaseApi } from '../../src/services/api';
import {
  getStoredDriveToken,
  syncDocumentToDrive,
} from '../../src/services/googleDriveFiles';
import { syncHearingToCalendar } from '../../src/services/calendarSync';
...
export default function CaseDetailScreen() {
  ...
  const {
    ...
    addHearing, updateHearing, deleteHearing, updateCase,
    addDocument, deleteDocument,
    sendWhatsAppMessage, sendSMSMessage, advocateName,
    isDriveConnected, connectDrive, updateDocumentDriveSync,
    setDocUploading, clearDocUpload, setDocUploadFailed, isDocUploading, isDocUploadFailed,
    calendarSyncEnabled, deviceCalendarId, hearingCalendarEventIds, setHearingCalendarEventId,
    isProUser, isFirmOwner, firm, authToken,
  } = useApp();
  ...
  // State flag: open doc picker after DriveSetupSheet dismisses
  const [pendingDocPick, setPendingDocPick] = useState(false);

  // Open doc sheet when drive sheet fully closes
  useEffect(() => {
    if (!showDriveSheet && pendingDocPick) {
      setShowDocSheet(true);
      setPendingDocPick(false);
    }
  }, [showDriveSheet, pendingDocPick]);
  ...
  const firmMembers: any[] = firm ? ((firm as any).members ?? []).filter((m: any) => m.role !== 'owner') : [];

  const handleAssignCase = async () => {
    if (!selectedAssignee || !caseData) return;
    setAssigning(true);
    try {
      await assignCaseApi(caseData.id, selectedAssignee, authToken ?? undefined);
      setShowAssignModal(false);
      setSelectedAssignee(null);
      Alert.alert('Assigned', 'Case assigned successfully');
    } catch (err: any) {
      Alert.alert('Error', err.message || 'Failed to assign case');
    } finally {
      setAssigning(false);
    }
  };
  ...
  const handleSendHearingReminder = () => {
    ...
    if (!client.phone) {
      if (Platform.OS === 'web') {
        // Alert.alert with buttons is blocked in browser — use window.alert for feedback
        (globalThis as any).alert?.(`${client.name} has no phone number. Add it in the Clients tab to send WhatsApp reminders.`);
      } else {
        Alert.alert(
  ...
  // Upload doc to Drive — uses global context tracking so indicator persists across navigation
  const uploadDocToDrive = async (docId: string, uri: string, fileName: string) => {
    ...
    try {
      ...
      if (!token) { clearDocUpload(docId); return; }
      ...
      clearDocUpload(docId);
    } catch {
      setDocUploadFailed(docId);
    }
  };
  ...
  const handleDeleteDocument = (docId: string, fileName: string) => {
    if (Platform.OS === 'web') {
      // window.confirm is blocked in browser iframes — delete directly on web (same pattern as Module 1/2/3/4/5)
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity
            ...
            onPress={isProUser ? handlePrint : () => openPaywall('PDF Reports')}
            disabled={printing}
          ...
          >
            ...
            {!isProUser && (
              <View style={styles.lockBadge} pointerEvents="none">
                <Feather name="lock" size={9} color={c.background} />
              </View>
            )}
          </TouchableOpacity>
      ...
      >
        ...
        {/* Firm Case Assignment — visible to firm owners */}
        {isFirmOwner && firmMembers.length > 0 && (
          <TouchableOpacity
            testID="assign-case-btn"
            style={styles.assignBtn}
            onPress={() => setShowAssignModal(true)}
            activeOpacity={0.8}
          >
            <Feather name="user-plus" size={14} color={c.textPrimary} />
            <Text style={styles.assignBtnText}>Assign to Team Member</Text>
          </TouchableOpacity>
        )}
          ...
          <View style={styles.sectionHeader}>
            ...
            <View style={styles.hearingActions}>
              ...
              <TouchableOpacity
                ...
                onPress={isProUser ? handleOpenEcourts : () => openPaywall('eCourts Lookup')}
                style={[styles.ecourtsBtn, !isProUser && { opacity: 0.75 }]}
                activeOpacity={0.8}
              >
                {!isProUser
                  ? <Feather name="lock" size={13} color="#1A73E8" />
                ...
                <View key={h.id} style={styles.hearingItem}>
                  ...
                  <View style={styles.hearingContent}>
                    <View style={styles.hearingTop}>
                      ...
                      {/* Hearing actions: edit, delete, calendar sync */}
                      <View style={{ marginLeft: 'auto', flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        {/* Edit hearing */}
                        <TouchableOpacity
                          hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
                          onPress={() => { setEditHearing(h); setShowAddHearingModal(true); }}
                          testID={`edit-hearing-${h.id}`}
                        >
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            if (Platform.OS === 'web') {
                              ...
                              Alert.alert('Delete Hearing', 'Remove this hearing?', [
                                { text: 'Cancel', style: 'cancel' },
                                { text: 'Delete', style: 'destructive', onPress: () => deleteHearing(h.id) },
                              ]);
                            }
                          }}
                          testID={`delete-hearing-${h.id}`}
                        >
                          <Feather name="trash-2" size={13} color={c.textSecondary} />
                        </TouchableOpacity>
                        {/* Phase 26 — per-hearing calendar icon */}
                        <TouchableOpacity
                          style={{ padding: 4 }}
                          hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
                          onPress={async () => {
                            if (!calendarSyncEnabled) {
                              if (Platform.OS !== 'web') {
                                Alert.alert('Calendar Sync', 'Enable calendar sync in Settings first.');
                              }
                              return;
                            }
                            if (hearingCalendarEventIds[h.id]) return;
                            try {
                              const eventId = await syncHearingToCalendar({
                                ...
                                courtName: caseData?.courtName || '',
                                hearingDate: new Date(h.hearingDate).toISOString(),
                                hearingPurpose: h.purpose,
                              });
                              setHearingCalendarEventId(h.id, eventId);
                            } catch {}
                          }}
                          testID={`calendar-icon-${h.id}`}
                        >
                          <Feather
                            name="calendar"
                            size={13}
                            color={
                              hearingCalendarEventIds[h.id]
                                ? '#34C759'
                                : calendarSyncEnabled
                                  ? c.textSecondary
                                  : c.textTertiary
            ...
            <View style={styles.docsList}>
              {documents.map(doc => (
                ...
                >
                  ...
                  {/* Drive sync status — uses global upload tracking */}
                  {isDocUploading(doc.id) ? (
                    <ActivityIndicator size="small" color={c.textPrimary} testID={`uploading-doc-${doc.id}`} />
                  ) : doc.googleDriveFileId ? (
                    <Feather name="cloud" size={16} color="#4285F4" />
                  ) : (
                    // No Drive file — either failed or never uploaded; show retry icon
                    <TouchableOpacity
                      hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                      onPress={() => uploadDocToDrive(doc.id, doc.uri ?? '', doc.fileName)}
                      testID={`retry-doc-${doc.id}`}
                    >
                      <Feather
                        name={isDocUploadFailed(doc.id) ? 'refresh-cw' : 'upload-cloud'}
                        size={16}
                        color={isDocUploadFailed(doc.id) ? '#FF3B30' : '#FF9500'}
                      />
                    </TouchableOpacity>
                  )}
                </TouchableOpacity>
        ...
        {/* Delete Button */}
        <TouchableOpacity style={styles.deleteBtn} onPress={handleDelete} testID="delete-case-btn">
      ...
      <DriveSetupSheet
        ...
        onDismiss={() => { setShowDriveSheet(false); setPendingDocPick(true); }}
      />
      {/* Phase 27 — Paywall */}
      <PaywallSheet
        visible={showPaywall}
      ...
      >
        ...
        >
          <View style={styles.assignModal}>
            <Text style={styles.assignModalTitle}>Assign to Team Member</Text>
            {firmMembers.map((m: any) => (
              <TouchableOpacity
                key={m.advocateId}
                testID={`assign-member-${m.advocateId}`}
                style={[
                  styles.assignMemberRow,
                  selectedAssignee === m.advocateId && styles.assignMemberRowSelected,
                ]}
                onPress={() => setSelectedAssignee(m.advocateId)}
              >
                <Text style={styles.assignMemberPhone}>{m.phone}</Text>
                {selectedAssignee === m.advocateId && (
            ...
            <TouchableOpacity
              ...
              style={[styles.assignConfirmBtn, !selectedAssignee && { opacity: 0.5 }]}
            ...
            >
              {assigning ? (
                ...
                <Text style={styles.assignConfirmText}>Assign Case</Text>
              )}
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
...
const makeStyles = (c: ColorPalette) => StyleSheet.create({
  ...
  assignBtnText: { ...Typography.subhead, color: c.textPrimary },
  assignModal: {
    backgroundColor: c.background, borderTopLeftRadius: Radius.l, borderTopRightRadius: Radius.l,
    padding: Spacing.l, paddingBottom: 40,
  },
  assignModalTitle: { ...Typography.headline, fontWeight: '700', color: c.textPrimary, marginBottom: Spacing.m },
  assignMemberRow: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingVertical: 12, paddingHorizontal: Spacing.m,
    borderRadius: Radius.m, marginBottom: Spacing.xs,
    backgroundColor: c.surface,
  },
  assignMemberRowSelected: { backgroundColor: c.surface, borderWidth: 1.5, borderColor: c.textPrimary },
  assignMemberPhone: { ...Typography.body, color: c.textPrimary },
  assignConfirmBtn: {
    backgroundColor: c.textPrimary, borderRadius: Radius.m,
    paddingVertical: 14, alignItems: 'center', marginTop: Spacing.m,
  },
  assignConfirmText: { ...Typography.headline, color: c.background },
});
