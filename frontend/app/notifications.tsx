...
import { useApp, AppNotification, InboxNotification } from '../src/context/AppContext';
...
type UnifiedNotif = { id: string; title: string; body: string; type: string; isRead: boolean; ts: number; caseId?: string; isInbox?: boolean };

type ListItem =
  | { type: 'header'; label: string; key: string }
  | { type: 'item'; data: UnifiedNotif; key: string };

export default function NotificationsScreen() {
  const c = useColors();
  const s = useMemo(() => makeStyles(c), [c]);
  const router = useRouter();
  const { notifications, markNotificationRead, inboxNotifications, markInboxNotificationRead } = useApp();

  // Build unified list from both local and inbox notifications
  const allNotifs = useMemo((): UnifiedNotif[] => {
    const local: UnifiedNotif[] = notifications.map(n => ({
      id: n.id, title: n.title, body: n.body, type: n.type,
      isRead: !!n.readAt, ts: n.createdAt, caseId: n.caseId, isInbox: false,
    }));
    const inbox: UnifiedNotif[] = inboxNotifications.map(n => ({
      id: n.id, title: n.title, body: n.body, type: n.type,
      isRead: n.read, ts: new Date(n.createdAt).getTime(), isInbox: true,
    }));
    return [...inbox, ...local].sort((a, b) => b.ts - a.ts);
  }, [notifications, inboxNotifications]);

  const [readIds, setReadIds] = useState<Set<string>>(() =>
    new Set(allNotifs.filter(n => n.isRead).map(n => n.id))
  );

  const isRead = (id: string) => readIds.has(id);

  const handleTap = (n: UnifiedNotif) => {
    setReadIds(prev => new Set([...prev, n.id]));
    if (n.isInbox) {
      markInboxNotificationRead(n.id);
      if (n.type === 'firm_invite') router.push('/firm' as any);
    } else {
  ...
  const handleMarkAllRead = () => {
    ...
    setReadIds(all);
    allNotifs.forEach(n => {
      if (n.isInbox) markInboxNotificationRead(n.id);
      else markNotificationRead(n.id);
    });
  };

  const unread = useMemo(() => allNotifs.filter(n => !isRead(n.id)).length, [allNotifs, readIds]);

  // Build grouped list: Today | Earlier
  const listData = useMemo((): ListItem[] => {
    const todayCutoff = Date.now() - 86400000;
    const today = allNotifs.filter(n => n.ts >= todayCutoff);
    const earlier = allNotifs.filter(n => n.ts < todayCutoff);
          ...
          return (
            ...
            >
              ...
              <View style={s.info}>
                ...
                <Text style={s.time}>{timeAgo(n.ts)}</Text>
              </View>
            </TouchableOpacity>
          );
        }}
