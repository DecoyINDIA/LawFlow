      ...
      try {
        ...
        if (token) {
          try {
            ...
            if (res.success) {
              const pd = res.data as Record<string, unknown>;
              const done = !!(pd?.name && pd?.enrollmentNumber && pd?.barCouncil);
              router.replace(done ? '/(tabs)' : '/login');
              return;
            }
          } catch (err: any) {
            if (err?.status === 401) {
            ...
            // Network error — check local profile completeness
            try {
